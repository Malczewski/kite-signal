import { isDirectionInRange, angularDistanceOutsideRange } from './direction.js';
import type { ForecastPoint, PointScore, Rating, SpotKnowledge, WindowResult } from './types.js';
...
function directionScore(spot: SpotKnowledge, windDirDeg: number): { score: number; hazard: boolean } {
  if (spot.dangerousWindDirRanges.some((range) => isDirectionInRange(windDirDeg, range))) {
    return { score: 0, hazard: true };
  }
  if (isDirectionInRange(windDirDeg, spot.idealWindDirRange)) {
    return { score: 1, hazard: false };
  }
  if (isDirectionInRange(windDirDeg, spot.usableWindDirRange)) {
    return { score: 0.6, hazard: false };
  }
...
function speedScore(spot: SpotKnowledge, windSpeedKts: number): number {
  ...
  if (windSpeedKts < spot.minWindKts || windSpeedKts > spot.maxWindKts) return 0;
  if (windSpeedKts >= idealMin && windSpeedKts <= idealMax) return 1;
